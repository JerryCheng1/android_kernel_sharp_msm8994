	...
	int16_t next_lens_position, uint32_t hw_params, uint16_t delay)
{
	...
	reg_sec_setting.reg_addr = ((next_lens_position & 0x0300) >> 8) | 0xC4;
	reg_sec_setting.reg_data[0] = (next_lens_position & 0x00FF);

	reg_sec_setting.reg_data_size = 1;

	CDBG("reg_sec_setting.reg_data[0]:0x%x\n", reg_sec_setting.reg_data[0]);
	rc = a_ctrl->i2c_client.i2c_func_tbl->
...
/* SHLOCAL_CAMERA_DRIVERS-> */
static int32_t bu64291_act_i2c_write(struct msm_actuator_ctrl_t *a_ctrl,
	void __user *argp)
{
...
static int32_t bu64291_act_i2c_read(struct msm_actuator_ctrl_t *a_ctrl,
	void __user *argp)
{
	struct i2c_info_t *cdata = (struct i2c_info_t *)argp;
	int32_t rc = -EFAULT;
...
/* SHLOCAL_CAMERA_DRIVERS-> */
	...
	CDBG("%s:%d SHCFG_SET_I2C_DATA\n", __func__, __LINE__);
		rc = bu64291_act_i2c_write(a_ctrl, &cdata->cfg.i2c_info);
		if (rc < 0)
	...
	CDBG("%s:%d SHCFG_GET_I2C_DATA\n", __func__, __LINE__);
		rc = bu64291_act_i2c_read(a_ctrl, &cdata->cfg.i2c_info);
		if (rc < 0)

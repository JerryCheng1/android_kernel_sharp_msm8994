/* drivers/sharp/shdisp_lcdc/shdisp_bl71y6.c  (Display Driver)
 *
...
#include "./data/shdisp_bl71y6_cmn.h"
#include "./data/shdisp_bl71y6_data_gp11d.h"
#include "./data/shdisp_bl71y6_led_gp11d.h"
...
static int shdisp_bdic_LD_PHOTO_SENSOR_get_lux(unsigned short *value, unsigned int *lux);
...
static int shdisp_bdic_PD_set_hw_reset(void);
...
int shdisp_bdic_API_boot_init( void )
{
    ...
    if (ret != SHDISP_RESULT_SUCCESS || (version & 0x08) != 0x08) {
        SHDISP_ERR("Version Read. ret=%d Version=%02X", ret, version);
        shdisp_bdic_PD_set_hw_reset();
        return SHDISP_BDIC_IS_NOT_EXIST;
    }
#endif /* SHDISP_NOT_SUPPORT_NO_OS */
...
{
    shdisp_SYS_set_Host_gpio(rst_gpio, SHDISP_GPIO_CTL_LOW);
    shdisp_SYS_delay_us(15000);
    shdisp_SYS_set_Host_gpio(rst_gpio, SHDISP_GPIO_CTL_HIGH);
...
static void shdisp_bdic_set_bkl_mode(unsigned char bkl_mode, unsigned char data, unsigned char msk)
{
    ...
    SHDISP_DEBUG("bkl_mode=0x%02x", value[0]);
...
void shdisp_bdic_API_set_lux_mode(unsigned char lux_mode, unsigned char data, unsigned char msk)
{
    ...
    SHDISP_DEBUG("lux_mode=0x%02x", value[0]);
...
void shdisp_bdic_API_set_lux_mode_modify(unsigned char data, unsigned char msk)
{
    ...
    SHDISP_DEBUG("before lux_mode=0x%02x", lux_mode[0]);

    lux_mode[0] = lux_mode[0] & ~msk;
    lux_mode[0] |= data;

    SHDISP_DEBUG("after lux_mode=0x%02x", lux_mode[0]);
...
void shdisp_bdic_API_RECOVERY_lux_data_backup(void)
{
    ...
    SHDISP_DEBUG("backup bkl_mode_recovery = 0x%02x", bkl_mode_recovery);
    SHDISP_DEBUG("backup lux_mode_recovery = 0x%02x", lux_mode_recovery);
...
{
    SHDISP_DEBUG("restore bkl_mode_recovery = 0x%02x", bkl_mode_recovery);
    SHDISP_DEBUG("restore lux_mode_recovery = 0x%02x", lux_mode_recovery);
...
static int shdisp_bdic_LD_PHOTO_SENSOR_get_lux(unsigned short *value, unsigned int *lux)
{
    ...
    if (ado != 0) {
        for (i = 0; i < SHDISP_BDIC_LUX_TABLE_ARRAY_SIZE; i++) {
            if ((ado >= shdisp_bdic_bkl_ado_tbl[i].range_low) &&
                ...
                ret_lux  = ado * (unsigned short)shdisp_bdic_bkl_ado_tbl[i].param_a;
   ...
   *lux = ret_lux;
    ...
    SHDISP_TRACE("out ado=0x%04X, lux=%u", ado, ret_lux);
    return SHDISP_RESULT_SUCCESS;
}
...
static int shdisp_bdic_LD_PHOTO_SENSOR_get_als(unsigned short *clear, unsigned short *ir)
{
    ...
    *ir = (unsigned short)((InfoReg1 >> 16) & 0x0000ffff);
    ...
    SHDISP_TRACE("out clear=0x%04x, ir=0x%04x", *clear, *ir);
    return SHDISP_RESULT_SUCCESS;
}
...
static int shdisp_bdic_LD_i2c_write(struct shdisp_bdic_i2c_msg *msg)
{
    ...
    if (msg->wlen > SHDISP_BDIC_I2C_WBUF_MAX) {
        SHDISP_ERR("<INVALID_VALUE> msg->wlen(%d).", msg->wlen);
        return SHDISP_RESULT_FAILURE;
    }
...
static int shdisp_bdic_LD_i2c_read_mode0(struct shdisp_bdic_i2c_msg *msg)
{
    ...
    if (msg->rlen > SHDISP_BDIC_I2C_RBUF_MAX) {
        SHDISP_ERR("<INVALID_VALUE> msg->rlen(%d).", msg->rlen);
        return SHDISP_RESULT_FAILURE;
    }
...
static int shdisp_bdic_PD_set_active(int power_status)
{
    ...
    if (power_status == SHDISP_DEV_STATE_NOINIT) {
        ...
        SHDISP_DEBUG("Read bdic version(0x%x)", version);
...
static void shdisp_bdic_PD_REG_ADO_get_opt(unsigned short *value)
{
    ...
    *value = (unsigned short)(InfoReg2 & 0x0000ffff);
    ...
    SHDISP_DEBUG("value=0x%04x", *value);
...
static int shdisp_bdic_PD_wait4i2ctimer_stop(void)
{
    ...
    do {
        ...
        if (ret != SHDISP_RESULT_SUCCESS) {
            SHDISP_ERR("retry(%d)!! SYSTEM8 read error.", waitcnt);
            continue;
        }
...
static int shdisp_bdic_PD_get_auto_bkl_level(void)
{
    ...
    SHDISP_TRACE("out ADO=%04X, bkl_value=%d", ado_value, ret_value);
    return ret_value;
}

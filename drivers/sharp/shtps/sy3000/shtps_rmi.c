...
static void shtps_deinit_inputdev(struct shtps_rmi_spi *ts);
	...
	static void shtps_cover_set_state(struct shtps_rmi_spi *ts, u8 request);
...
void shtps_ioctl_setlcdbrightlpmode_proc(struct shtps_rmi_spi *ts, u8 on)
{
	...
	shtps_set_lpmode(ts, SHTPS_LPMODE_TYPE_NON_CONTINUOUS, SHTPS_LPMODE_REQ_LCD_BRIGHT, (int)on);
		...
		if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) == 0 ){
			shtps_set_fw_report_rate(ts, SHTPS_CTRL_FW_REPORT_RATE_PARAM_INIT);
		}else{
			shtps_set_fw_report_rate(ts, SHTPS_CTRL_FW_REPORT_RATE_PARAM_LCD_BRIGHT_LOW);
		}
...
static void shtps_cover_proc(struct shtps_rmi_spi *ts, u8 request)
{
	#if defined(SHTPS_COVER_ENABLE)
		SHTPS_LOG_FUNC_CALL();
		shtps_cover_set_state(ts, request);
	#endif /* SHTPS_COVER_ENABLE */
}
...
void shtps_set_fw_report_rate(struct shtps_rmi_spi *ts, u8 rate)
{
	int ret;
	u8 set_rate = rate;
	...
	if(set_rate == 0){
		ret = shtps_set_fw_report_rate_init(ts);
	}else{
...
static void shtps_lpwg_wakelock_init(struct shtps_rmi_spi *ts)
{
	...
	ts->lpwg.block_touchevent = 0;
		...
		ts->lpwg.grip_wake_lock_state = 0;
	    wake_lock_init(&ts->lpwg.grip_wake_lock, WAKE_LOCK_SUSPEND, "shtps_lpwg_grip_wake_lock");
		ts->lpwg.grip_pm_qos_lock_idle.type = PM_QOS_REQ_AFFINE_CORES;
		ts->lpwg.grip_pm_qos_lock_idle.cpus_affine.bits[0] = 0x0f;
		pm_qos_add_request(&ts->lpwg.grip_pm_qos_lock_idle, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
	#else
...
static void shtps_lpwg_wakelock_destroy(struct shtps_rmi_spi *ts)
{
   ...
   pm_qos_remove_request(&ts->lpwg.pm_qos_lock_idle);
	...
	#if defined(SHTPS_LPWG_GRIP_SUPPORT_ENABLE)
		wake_lock_destroy(&ts->lpwg.grip_wake_lock);
		pm_qos_remove_request(&ts->lpwg.grip_pm_qos_lock_idle);
	#endif /* SHTPS_LPWG_GRIP_SUPPORT_ENABLE */
}
...
void shtps_lpwg_grip_wakelock(struct shtps_rmi_spi *ts, int on)
{
	SHTPS_LOG_FUNC_CALL();
	if(on){
		if(ts->lpwg.grip_wake_lock_state == 0){
			ts->lpwg.grip_wake_lock_state = 1;
		    wake_lock(&ts->lpwg.grip_wake_lock);
		    pm_qos_update_request(&ts->lpwg.grip_pm_qos_lock_idle, SHTPS_LPWG_QOS_LATENCY_DEF_VALUE);
			SHTPS_LOG_DBG_PRINT("lpwg grip wake lock ON\n");
		}
	}else{
		if(ts->lpwg.grip_wake_lock_state != 0){
			ts->lpwg.grip_wake_lock_state = 0;
			wake_unlock(&ts->lpwg.grip_wake_lock);
		    pm_qos_update_request(&ts->lpwg.grip_pm_qos_lock_idle, PM_QOS_DEFAULT_VALUE);
			SHTPS_LOG_DBG_PRINT("lpwg grip wake lock OFF\n");
		}
	}
}
...
static int shtps_init_param(struct shtps_rmi_spi *ts)
{
	...
	#if defined(SHTPS_CHANGE_TRANSITION_TUNING_ENABLE)
		rc = shtps_fwctl_initparam_transition_tuning(ts);
		SPI_ERR_CHECK(rc, err_exit);
	#endif /* SHTPS_CHANGE_TRANSITION_TUNING_ENABLE */
...
void shtps_event_report(struct shtps_rmi_spi *ts, struct shtps_touch_info *info, u8 event)
{
	...
	if( (event == SHTPS_EVENT_TD) && (ts->touch_state.numOfFingers == 1) ){
		memcpy(&ts->report_info_single_td, info, sizeof(ts->report_info_single_td));
	}
...
void msm_tps_get_single_touchdown_info(int *x, int *y)
{
	struct shtps_rmi_spi *ts = gShtps_rmi_spi;
	int i;
	int fingerMax = shtps_get_fingermax(ts);
	...
	if( (x == NULL) || (y == NULL) ){
		SHTPS_LOG_DBG_PRINT("%s() Argument pointer is NULL\n", __func__);
		return;
	}
	...
	*y = -1;
	...
	shtps_mutex_lock_ctrl();
	for(i = 0;i < fingerMax;i++){
		if( (ts->report_info_single_td.fingers[i].state == SHTPS_TOUCH_STATE_FINGER) ||
			(ts->report_info_single_td.fingers[i].state == SHTPS_TOUCH_STATE_PEN) )
		{
			*x = ts->report_info_single_td.fingers[i].x;
			*y = ts->report_info_single_td.fingers[i].y;
			break;
		}
	}
	shtps_mutex_unlock_ctrl();

	SHTPS_LOG_DBG_PRINT("%s() [x=%d, y=%d]\n", __func__, *x, *y);
}
EXPORT_SYMBOL(msm_tps_get_single_touchdown_info);
